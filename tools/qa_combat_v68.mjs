...
for (const region of regions) {
  ...
  await page.goto(`http://127.0.0.1:4175/admin/dungeon-flow.html?region=${region}&v=74.0.0`, { waitUntil:'networkidle' });
...
const report = { version:'74.0.0', regions:results, allPassed:results.every(r => r.victory && r.completed && r.playerHp > 0 && r.actions <= 18 && r.companion.includes('검수 동행') && r.deployedCards===5 && r.raidUnits===5 && r.raidVertical && r.knots===7 && r.railVisible && r.rewards===4 && r.manifestActor && r.ainVisible && r.skillBannerSeen && r.criticalSeen && r.seventhMotionSeen && r.criticalGold && r.errors.length === 0) };
