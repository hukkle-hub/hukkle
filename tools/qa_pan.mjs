...
for(let i=0;i<7;i++){const hit=await act();check(hit.after.knotsRemaining===6-i&&hit.after.playerHp===100,'clash cancellation');}
...
check(embedded.src.includes('/dungeon-flow.html?')&&embedded.nested===0&&embedded.version==='81.1.0'&&embedded.party==='5','embedded integration '+JSON.stringify(embedded));
await page.screenshot({path:join(out,'embedded-battle.png')});
results.push({embedded,passed:true});
}catch(e){errors.push(String(e));}finally{await writeFile(join(out,'report.json'),JSON.stringify({results,errors},null,2));console.log(JSON.stringify({results,errors}));await browser.close();server.close();if(results.length!==6||errors.length)process.exitCode=1;}
