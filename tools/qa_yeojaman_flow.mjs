...
await page.goto('http://127.0.0.1:4174/admin/dungeon-flow.html?region=yeoja&v=66.0.0', { waitUntil: 'networkidle' });
...
const completed = await page.evaluate(() => JSON.parse(localStorage.getItem('hy-dungeon-flow-v660') || '{}')?.yeoja?.complete === true);
const report = { phases, markerChecks, clueCount, dungeonEnabled, zoneCount, bossEnabled, layout, completed, errors };
...
if (!hdScenes || markerChecks.length !== 3 || !markerChecks.every(x => x.visible) || clueCount?.trim() !== '기록 3 / 3' || !dungeonEnabled || zoneCount !== 3 || !bossEnabled || !Object.values(layout).every(Boolean) || !completed || errors.length) process.exit(1);
